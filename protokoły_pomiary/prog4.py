...
def generuj_protokol():
    ...
    rodzaj_sieci = input("Podaj rodzaj sieci (np. TN-C-S, TN-S, TN-C, TT): ").upper().strip()
    typ_pomiaru = input("Podaj typ pomiaru (L-PE, L-N, L-PEN): ").upper().strip()
    ...
    if rodzaj_sieci == 'TN-C' and typ_pomiaru != 'L-PEN':
        ocena_typ_pomiaru = "Błąd: W układzie TN-C należy wykonywać pomiar L-PEN."
    elif rodzaj_sieci in ['TN-S', 'TN-C-S', 'TT'] and typ_pomiaru != 'L-PE':
        ocena_typ_pomiaru = f"Błąd: W układzie {rodzaj_sieci} należy wykonywać pomiar L-PE."
    else:
        ocena_typ_pomiaru = "Pomiar zgodny z układem sieci."
...
--- Weryfikacja ---
Weryfikacja typu pomiaru: {ocena_typ_pomiaru}
