...
class ProtocolGeneratorApp(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(100, 100, 700, 750)  # x, y, width, height
    ...
    def init_ui(self):
        ...
        self.measurement_type_combo.addItems(["L-PE", "L-N", "L-PEN"])
        ...
        self.add_result_field(results_layout, "Weryfikacja typu pomiaru:", "measurement_type_verification_label", "",
                              bold=True)
    ...
    def add_input_field(self, layout, label_text, attr_name, placeholder_text, numeric=False):
        ...
        label.setFixedWidth(200)  # Stała szerokość dla wyrównania
        h_layout.addWidget(label)

        line_edit = QLineEdit()
        line_edit.setPlaceholderText(placeholder_text)
        if numeric:
            line_edit.setValidator(self.create_float_validator())  # Użyj walidatora dla liczb zmiennoprzecinkowych
    ...
    def add_label_to_layout(self, layout, text):
        ...
        h_layout.addWidget(label)
        h_layout.addStretch()  # Wyrównaj tekst do lewej
        layout.addLayout(h_layout)
    ...
    def add_result_field(self, layout, label_text, attr_name, unit="", bold=False):
        ...
        value_label.setTextInteractionFlags(Qt.TextSelectableByMouse)  # Umożliwia kopiowanie
        if bold:
    ...
    def create_float_validator(self):
        # Prosty walidator, który pozwala na wprowadzenie liczb zmiennoprzecinkowych
        # W PySide6 można użyć QDoubleValidator, ale dla prostoty tutaj używamy walidacji podczas konwersji
        return None  # QLineEdit.text() będzie potem konwertowane na float
    ...
    def generate_protocol(self):
        ...
        if rodzaj_sieci == 'TN-C' and typ_pomiaru != 'L-PEN':
...
"""
    ...
    def reset_results(self):
        # Funkcja do resetowania wyników w GUI w przypadku błędu
